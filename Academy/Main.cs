...
namespace Academy
{
	public partial class Main : Form
	{
		...
		private void ShowEmptyDirections_CheckedChanged(object sender, EventArgs e)
		{
			if(ShowEmptyDirections.Checked == true)
			{
				int i = tabControl.SelectedIndex;
				Query query = queries[i];
				tables[i].DataSource =
			...
			if(ShowEmptyDirections.Checked == false)
			{
				dgvDirections.DataSource = connector.Select
						(
						"direction_name, COUNT(DISTINCT group_id) AS N'Количество групп', COUNT(stud_id) AS N'Количество студентов'",
		...
		private void cbStudentsDirection_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			Dictionary<string, int> d_groups = connector.GetDictionary
				...
				"group_id, group_name",
				"Groups",
				i == 0 ? "" : $"direction = {cbStudentsDirection.SelectedItem.ToString()}"
				);
			cbStudentsGroup.Items.Clear();
			cbStudentsGroup.Items.AddRange(d_groups.Select(g => g.Key).ToArray());

			//i = tabControl.SelectedIndex;
			//dgvStudents.DataSource = 
			//	connector.Select
			//	(
			//		queries[i].Columns,
			//		queries[i].Tables,
			//		i == 0 ? "" : $"direction = {d_directions[cbGroupsDirection.SelectedItem.ToString()]}"
			//	);
			Query query = new Query(queries[0]);
			query.Condition = (i == 0 || cbStudentsDirection.SelectedItem == null ? "" : $"direction = {d_directions[cbGroupsDirection.SelectedItem.ToString()]}");
			LoadPage(0, query);
		}
	}
}